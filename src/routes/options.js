const UserActions = [],
    AdminActions = ["create", "getRaw", "list", "listAndCount", "listAll"],
    MODEL_NAME = "Options",
    MODEL_OPTIONS = {
        MODEL_NAME,
        MODEL_TITLE: "Опции",
...
const getIP = notNode.Common.getIP;
...
async function updateDocument(doc) {
    ...
    if (!item) {
        throw new notError(notLocale.say("document_not_found"), {
            doc,
        });
    }

    let updated = await thisModel
        .findOneAndUpdate(
            {
                _id: doc._id,
            },
...
exports._updateForModule = async (req, res) => {
    ...
    try {
        ...
        const notApp = notNode.Application;
        const result = await notApp
            .getModel(MODEL_NAME)
            .writeModuleOptions(moduleName, options);
